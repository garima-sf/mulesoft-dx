...
Diff each Mode-B `.attributes[].attributeName` against the same site's `usage.usage_sites[].attributes_set` keys — a rename is a `usage` key absent from `.attributes[]` AND a `.attributes[]` name absent from `usage`. Diff `usage.errorTypes_caught[]` against `<nick>-new-<op>.json .errorTypes[]` and `<nick>-new.json .errorTypes[]` — a caught type that isn't in either is a mapping the plan must resolve. Diff Mode-C `.elementName` and `.connectionProviders[].elementName` against `usage.configs_used[]` and `usage.config_providers_used[]` — a mismatched local-name is a config-element rewrite the plan must enumerate. This is a **provider/config element rename** (the element's OWN local-name changing), distinct from the child-tree diff below — the child-tree diff produces zero residue when the provider's children are unchanged, so it will NOT catch this. Apply it as **set membership, after case-normalization**:

- Build the NEW set = union of every `.connectionProviders[].elementName` across ALL of this connector's `<nick>-new-<config>-<provider>.json` files (a config can expose several providers; you need the whole set to distinguish *renamed* from *removed* and to find the survivor).
- Take the OLD local-name from `usage.config_providers_used[]` (the connection element the flow declares inside `<prefix:config>`).
- **Fold both sides to one case form before comparing** — Mode-C `elementName` is kebab-case while `usage.config_providers_used[]` is camelCase. Comparing raw would false-positive on connectors that DID NOT rename (a provider whose two forms are just kebab-vs-camel of the same name).
- If the OLD local-name is **absent from the NEW set**, the provider element was renamed → enumerate the `<prefix:config>` child rewrite to the surviving element in §7. If present, no rewrite. A missed rename here is a guaranteed `mule-maven-plugin:process-classes` XSD failure (`cvc-complex-type.2.4.a`), which the captured Mode-C metadata already predicts.
