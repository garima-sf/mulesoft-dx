...
- <javaVersion> / maven.compiler.{source,target,release} / java.version / jdk.version: <from> → <to> (each bumped only if present; never inserted)
- maven.compiler.release: **inserted** = <to> **only when** the app has custom Java (`src/main/java` or `src/test/java`) AND no compiler level is declared anywhere (property or `maven-compiler-plugin` config) — otherwise the custom Java won't compile on the target JDK (which rejects the ancient default source level). List this bullet in the plan when that condition holds; omit it for pure flow/DataWeave apps or when a level is already declared.
...
- **Java version properties** — `<javaVersion>`, `maven.compiler.{source,target}`, `maven.compiler.release`, `java.version`, `jdk.version` are each bumped **only if already present** (never inserted — a POM that doesn't declare a tag shouldn't sprout it). **One exception:** when the app has custom Java sources (`src/main/java`/`src/test/java`) and no compiler level is declared anywhere (property or `maven-compiler-plugin` config), `maven.compiler.release` is **inserted** = target Java. Without it, javac falls back to an ancient default source level that the target JDK rejects (`Source option 5 is no longer supported`), failing the build. Pure flow/DataWeave/MUnit-XML apps have no Java to compile, so nothing is inserted.
